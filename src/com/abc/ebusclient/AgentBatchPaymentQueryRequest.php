...
class AgentBatchPaymentQueryRequest extends TrxRequest {
	...
	function __construct() {
		$this->request["TrxType"] = IFunctionID :: TRX_TYPE_EBUS_AGENTBATCHQUERY_RESULT;
	}
	...
	/// <summary>
        ...
        public function getBatchSatusChinese($aStatus)
        {
            ...
            else if ($aStatus === (IBatchStatus ::STATUS_CHECKSUCCESS))
            {
                $tStatusChinese = "批量复核通过待发送";
            }
            else if ($aStatus === (IBatchStatus ::STATUS_REJECT))
            {
                $tStatusChinese = "批量复核被驳回";
            }
            else if ($aStatus === (IBatchStatus ::STATUS_SEND))
            {
                $tStatusChinese = "批量等待处理";
            }
            else if ($aStatus === (IBatchStatus ::STATUS_SUCCESS))
            {
                $tStatusChinese = "批量提交成功";
            }
            else if ($aStatus === (IBatchStatus ::STATUS_FAIL))
            {
